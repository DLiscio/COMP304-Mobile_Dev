...
class EditTask : ComponentActivity() {
...
@Composable
fun Material3DesignET(content: @Composable () -> Unit) {
...
@Composable
fun EditTaskPage(taskViewModel : TaskViewModel) {
    ...
    Scaffold(
        ...
        floatingActionButton = { NewTaskFABET(taskTitle = taskTitle, taskDueDate = formattedDate, taskViewModel = taskViewModel) },
        bottomBar = { BottomNavigationBarET() }
    ) { innerPadding ->
        EditTaskContent(
...
@Composable
fun NewTaskFABET(taskTitle: String, taskDueDate: String, taskViewModel: TaskViewModel) {
...
fun EditTaskContent(
    ...
    LazyColumn(
        ...
        item {
            ...
            val formattedDate = "${selectedDate.get(Calendar.YEAR)}-${selectedDate.get(Calendar.MONTH) + 1}-${selectedDate.get(
        ...
        item {
            ...
            Button(
                ...
                onClick = {
                    showDatePickerEdit(context) { newDate ->
...
fun showDatePickerEdit(context: Context, onDateSelected: (Calendar) -> Unit) {
